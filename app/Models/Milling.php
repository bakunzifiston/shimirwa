...
class Milling extends Model
{
    ...
    protected static function inferSource(string $type): string
    {
        return in_array(strtolower($type), ['soy', 'maize'], true) ? 'roasting' : 'sorting';
    }

    protected static function itemKey(array $item): string
    {
        $source = $item['source'] ?? self::inferSource((string) ($item['type'] ?? ''));

        return $source.':'.($item['stock_id'] ?? '');
    }

    protected static function resolveBatchFromItem(array $item, bool $lock = false): Roasting|Sorting|null
    {
        $stockId = (int) ($item['stock_id'] ?? 0);
        if (! $stockId) {
            return null;
        ...
        if ($source === 'roasting') {
            $query = Roasting::query();
            $batch = $lock ? $query->lockForUpdate()->find($stockId) : $query->find($stockId);
        } elseif ($source === 'sorting') {
            $query = Sorting::query()->with('rawMaterialStock');
            $batch = $lock ? $query->lockForUpdate()->find($stockId) : $query->find($stockId);
        } else {
            return null;
        }
    ...
    protected static function booted()
    {
        static::creating(function ($milling) {
            ...
            foreach ($items as $item) {
                ...
                $batch = self::resolveBatchFromItem($item);
                if (! $batch) {
                    throw new \Exception('Selected batch does not exist.');
                }

                if ($batch->remainingUsable() < $qty) {
                    throw new \Exception('Not enough stock in batch. Available: '.$batch->remainingUsable().' kg.');
                }
        ...
        static::created(function ($milling) {
            DB::transaction(function () use ($milling) {
                self::deductItemQuantities(self::normalizeMillingItems($milling->items));
            });
        });
